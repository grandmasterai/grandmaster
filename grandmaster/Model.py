...
class Model(Protocol):
    ...
    def get_task_signature(
        self, inputs: List[InputsTypedDict], outputs: List[OutputsTypedDict]
    ) -> Optional[Task]:
        # outdated
        for t in self.tasks():
            ...
            if (
                queryTypeToInputs(query) == inputs
                and resultToOutputs(result) == outputs
            ):
                return t
        return None

    def get_task(
        self, inputs: InputsTypedDict, outputs: OutputsTypedDict
    ) -> Optional[Task]:
        for t in self.tasks():
